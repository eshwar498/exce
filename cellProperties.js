...
italic.addEventListener("click", (e) => {
  ...
  selectedCell.style.fontStyle = selectedCellMatrix.italic
    ? "italic"
    : "normal";
  styleMenuBasedOnACell();
...
underline.addEventListener("click", (e) => {
  ...
  selectedCell.style.textDecoration = selectedCellMatrix.underline
    ? "underline"
    : "none";
  styleMenuBasedOnACell();
...
function styleMenuBasedOnACell() {
  let [selectedCell, selectedCellMatrix] = getCellAndCellInDB();
  bold.style.backgroundColor = selectedCellMatrix.bold
    ? activeColor
    : inactiveColor;
  bold.style.color = selectedCellMatrix.bold ? "white" : "black";
  italic.style.backgroundColor = selectedCellMatrix.italic
    ? activeColor
    : inactiveColor;
  italic.style.color = selectedCellMatrix.italic ? "white" : "black";
  underline.style.backgroundColor = selectedCellMatrix.underline
    ? activeColor
    : inactiveColor;
  ...
  fontFamily.value =
    selectedCellMatrix.fontFamily == null
      ? "monospace"
      : selectedCellMatrix.fontFamily;
  fontSize.value = selectedCellMatrix.fontSize;
  alignment.forEach((element, index) => {
    if (index == 0) {
      alignment[0].style.backgroundColor =
        selectedCellMatrix.alignment == "left" ? activeColor : inactiveColor;
      alignment[0].style.color =
        selectedCellMatrix.alignment == "left" ? "white" : "black";
    } else if (index == 1) {
      alignment[1].style.backgroundColor =
        selectedCellMatrix.alignment == "center" ? activeColor : inactiveColor;
      alignment[1].style.color =
        selectedCellMatrix.alignment == "center" ? "white" : "black";
    } else {
      alignment[2].style.backgroundColor =
        selectedCellMatrix.alignment == "right" ? activeColor : inactiveColor;
      alignment[2].style.color =
        selectedCellMatrix.alignment == "right" ? "white" : "black";
    }
  });
...
document.addEventListener("click", function (event) {
  const gridActionsContainer = document.querySelector(
