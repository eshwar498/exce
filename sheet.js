...
sheetIcon.addEventListener("click", () => sheetIconClick());
...
function updateAllCells(id) {
  for (let x = 0; x < row; x++) {
    for (let y = 0; y < col; y++) {
      ...
      cell.style.textDecoration = selectedCellMatrix.underline
        ? "underline"
        : "none";
      ...
      if (currSelectedSheet !== selectedSheetForCopyPaste)
        cell.style.border = "1px solid #afb0b3";
    }
  }
}

function updateCurrSheet(prevSelectedSheet, currentSheet) {
  let currSheet = document.getElementById(
    `sheet-${currentSheet || currSelectedSheet}`
  );
  if (prevSelectedSheet) {
    prevSelectedSheet.style.backgroundColor = "transparent";
...
function removeSheet(e) {
  ...
  if (nextAvailableSheet || nextAvailableSheet == 0)
    currSelectedSheet = nextAvailableSheet;
  if (nextAvailableSheet || nextAvailableSheet == 0)
    updateCurrSheet("", nextAvailableSheet);
  updateAllCells(currSelectedSheet);
}
