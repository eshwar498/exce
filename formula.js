  ...
  for (let y = 0; y < col; y++) {
    ...
    cells.addEventListener("click", () => {
      if (lastExitedCellAdd) {
        let [lastCell, lastCellInMatrix] =
          getCellAndCellInDB(lastExitedCellAdd);
...
formulaBar.addEventListener("keydown", (e) => {
  ...
  if (e.code == "Enter") {
    ...
    let getParentsFromFormula = changeFormulaToIncludeOnlyParentAddresses(
      formulaBar.value
    );
  ...
  for (let x = 0; x < arr.length; x++) {
    ...
    if (arr[x] != "+" && arr[x] != "-" && arr[x] != "*" && arr[x] != "/") {
      if (Number(arr[x])) valueWithinFormula = Number(arr[x]);
        ...
        if (addValues)
          selectedCellMatrix.children = selectedCellMatrix.children
            ? [...selectedCellMatrix.children, address]
            : [address];
      }
  ...
  if (!selectedCellMatrix.children) return;
    ...
    for (let x = 0; x < arr.length; x++) {
      ...
      if (currCellMatrix.formula) {
        let updatedCellValue = returnFormulaAnswer(
          currCellMatrix.formula,
          false
        );
    ...
    setTimeout(() => {
      ...
      if (index === cells.length - 1) {
        arr[index].style.backgroundColor =
          shouldAdd == "add" ? "orange" : "transparent";
        if (shouldAdd == "remove") {
          let response = confirm(
            "cycle will be formed.Do you want to trace it?"
          );
          if (response) {
        ...
        arr[index].style.backgroundColor =
          shouldAdd == "add" ? "blue" : "transparent";
        shouldAdd == "add"
          ? addAndRemoveHighLightsUsingSetTimeout(index + 1, "add")
          : addAndRemoveHighLightsUsingSetTimeout(index + 1, "remove");
      }
    }, 900);
  }
...
function removeParents(formula, address) {
  ...
  for (let x = 0; x < arr.length; x++) {
    let [currCell, currCellMatrix] = getCellAndCellInDB(arr[x]);
    let indexToDelete = currCellMatrix.children.findIndex(
