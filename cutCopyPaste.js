...
function selectedCell() {
  if (ctrlKey) {
    ...
    if (selectedCellRange.length >= 2) {
      let [selectedCell1, selectedCellMatrix1] = getCellAndCellInDB(
        selectedCellRange[0]
      );
      let [selectedCell2, selectedCellMatrix2] = getCellAndCellInDB(
        selectedCellRange[1]
      );
...
cutBtn.addEventListener("click", () => {
  ...
  if (selectedCellRange.length == 1) {
    console.log("cut 1");
    let [cellInUIToBePastedOn, cellInUIToBePastedOnMatrix] = getCellAndCellInDB(
      selectedCellRange[0],
      selectedSheetForCopyPaste
    );
  ...
  let [selectedStartColInNum, selectedStartRow] = decodeAddress(
    selectedCellRange[0]
  );
  let [selectedEndColInNum, selectedEndRow] = decodeAddress(
    selectedCellRange[1]
  );
  for (let x = selectedStartRow; x <= selectedEndRow; x++) {
    for (let y = selectedStartColInNum; y <= selectedEndColInNum; y++) {
      ...
      let [currSelectedCell, currSelectedCellInMatrx] = getCellAndCellInDB(
        currSelectedCellAddress,
        selectedSheetForCopyPaste
      );
  ...
  //    }
    ...
    for (let x = selectedStartRow; x <= selectedEndRow; x++) {
      for (let y = selectedStartColInNum; y <= selectedEndColInNum; y++) {
        ...
        let cellInUIToBePastedOnAddress = cellInUIToBePastedOn(
          selectedStartColInNum,
          selectedStartRow,
          x,
          y
        );
        updateCopiedCellsMatrix(
          cellInUIToBePastedOnAddress,
          currSelectedCellInMatrx
        );
        let [cellToBeCopiedOn, cellToBeCopiedOnInMatrix] = getCellAndCellInDB(
          cellInUIToBePastedOnAddress
        );
        if (!cellToBeCopiedOn) continue;
        updateASingleCellValueInUI(cellToBeCopiedOn, cellToBeCopiedOnInMatrix);
        updateCopiedCellsChildren(
          cellToBeCopiedOnInMatrix,
          y,
          x,
          cellInUIToBePastedOnAddress,
          selectedStartColInNum,
        ...
        if (isCutBtnClick) {
          allSheetsCellPropertiesMatrix[selectedSheetForCopyPaste][x - 1][
            y - 65
          ] = returnDefaultCellValue();
        }
      }
    }
    isCutBtnClick = false;
...
function onlyOneCellSelected() {
  ...
  let [copyFromCell, copyFromCellMatrix] = getCellAndCellInDB(
    selectedCellRange[0],
    selectedSheetForCopyPaste
  );
  ...
  allSheetsCellPropertiesMatrix[currSelectedSheet][copyToRow - 1][
    copyToCol - 65
  ].children;
  allSheetsCellPropertiesMatrix[currSelectedSheet][copyToRow - 1][
    copyToCol - 65
  ...
  if (isCutBtnClick) {
    allSheetsCellPropertiesMatrix[selectedSheetForCopyPaste][row - 1][
      col - 65
    ] = returnDefaultCellValue();
...
function updateCopiedCellsFormula(
  copiedCellInMatrix,
  currSelectedCellAddress,
  startCol,
  startRow,
  ...
  let formulasParents = changeFormulaToIncludeOnlyParentAddresses(
  ...
  formulasParents.forEach((parentAddress) => {
    ...
    if (
      parentCellCol < startCol ||
      parentCellCol > endCol ||
      parentCellRow < startRow ||
      parentCellRow > endRow
    ) {
      copiedCellInMatrix.formula = "";
  ...
  if (!areParentsInRange) return;
    ...
    let formulaArray = copiedCellInMatrix.formula
      .split(/\s*([\+\-\*/()/])\s*/)
      .filter(Boolean);
    ...
    for (let x = 0; x < formulaArray.length; x++) {
      if (
        ...
        formulaArray[x] == "*" ||
        formulaArray[x] == "/" ||
        !isNaN(formulaArray[x] - 1)
      ) {
        newFormulaString += formulaArray[x];
      } else {
        let [copiedCellAddressCol, copiedCellAddressRow] = decodeAddress(
          cellInUIToBePastedOnAddress
        );
        let [parentCol, parentRow] = decodeAddress(formulaArray[x]);
        let [currCellAddressCol, currCellAddressRow] = decodeAddress(
          currSelectedCellAddress
        );
        let colDiff = currCellAddressCol - parentCol;
        let rowDiff = currCellAddressRow - parentRow;
        let newAddress =
          String.fromCharCode(copiedCellAddressCol - colDiff) +
          (copiedCellAddressRow - rowDiff);
        if (
          newAddress.charCodeAt(0) > 90 ||
          newAddress.charCodeAt(0) < 65 ||
          Number(newAddress.slice(1, newAddress.length)) > 100 ||
          Number(newAddress.slice(1, newAddress.length)) < 0
        ) {
          copiedCellInMatrix.formula = "";
          return;
        }
...
function updateCopiedCellsChildren(
  copiedCellInMatrix,
  currSelectedCellColInNum,
  currSelectedRow,
  cellInUIToBePastedOnAddress,
  startCol,
  startRow,
  ...
  if (!copiedCellInMatrix.children || copiedCellInMatrix.children.length == 0)
    return;
  for (let x = 0; x < copiedCellInMatrix.children.length; x++) {
    let [childCellCol, childCellRow] = decodeAddress(
      copiedCellInMatrix.children[x]
    );
    if (
      childCellCol < startCol ||
      childCellCol > endCol ||
      childCellRow < startRow ||
      childCellRow > endRow
    ) {
    ...
    let newCellInUIToBePastedOnCol = String.fromCharCode(
      cellInUIToBePastedOnAddress.charCodeAt(0) + Number(colDiff)
    );
    let newCellInUIToBePastedOnRow =
      Number(
        cellInUIToBePastedOnAddress.slice(1, cellInUIToBePastedOnAddress.length)
      ) + Number(rowDiff);
    if (
      newCellInUIToBePastedOnCol.charCodeAt(0) > 90 ||
      newCellInUIToBePastedOnCol.charCodeAt(0) < 65 ||
      newCellInUIToBePastedOnRow > 100 ||
      newCellInUIToBePastedOnRow < 1
    ) {
      copiedCellInMatrix.children.splice(x, 1);
      continue;
    }
    let newCellAddress =
      newCellInUIToBePastedOnCol + newCellInUIToBePastedOnRow;
...
function updateASingleCellValueInUI(cellInUIToBePastedOn, copiedCellMatrix) {
  ...
  cellInUIToBePastedOn.style.fontWeight = copiedCellMatrix.bold
    ? "bold"
    : "normal";
  cellInUIToBePastedOn.style.fontStyle = copiedCellMatrix.italic
    ? "italic"
    : "normal";
  cellInUIToBePastedOn.style.textDecoration = copiedCellMatrix.underline
    ? "underline"
    : "none";
...
function cellInUIToBePastedOn(selectedStartColInNum, selectedStartRow, x, y) {
  let [cellToBePastedOnCol, cellInUIToBePastedOnRow] = decodeAddress(
    addressBar.value
  );
  cellToBePastedOnCol = cellToBePastedOnCol + (y - selectedStartColInNum);
  cellInUIToBePastedOnRow =
    Number(cellInUIToBePastedOnRow) + Number(x - selectedStartRow);
...
function updateCopiedCellsMatrix(cellInUIToBePastedOnAddress, cellInMatrix) {
  ...
  allSheetsCellPropertiesMatrix[currSelectedSheet][rid][cid] = JSON.parse(
    JSON.stringify(cellInMatrix)
  );
